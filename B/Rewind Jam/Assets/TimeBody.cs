...
public class TimeBody : MonoBehaviour
{
    ...
    void Record()
    {
        if ((rb.velocity.x > 0.1f || rb.velocity.x < -0.1f) || (rb.velocity.y > 0.1f || rb.velocity.y < -0.1f))
        {
            ...
            //}
            pointsInTime.Insert(0, new PointInTime(transform.position, GetComponent<PlayerScript>().spriteHolder.sprite, GetComponent<PlayerScript>().spriteHolder.flipX, GetComponent<PlayerScript>().facingRight));
        }
    }
    ...
    void Rewind()
    {
        if (pointsInTime.Count > 0)
        {
            ...
            GetComponent<PlayerScript>().spriteHolder.sprite = pointInTime.sprite;
            GetComponent<PlayerScript>().spriteHolder.flipX = pointInTime.flipX;
            GetComponent<PlayerScript>().facingRight = pointInTime.facingRight;
